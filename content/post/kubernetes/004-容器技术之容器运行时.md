...
&nbsp;&nbsp;&nbsp;&nbsp;Liz Rice在[`Building a container from scratch in Go`](http://blog.allposs.com/post/images/kubernetes/004/Building a container from scratch in Go - Liz Rice (Microscaling Systems).mp4)的演讲很好地介绍了如何实现low-level container runtimes。Liz通过许多步骤实现，但是一个最简单的运行时，仍然可以称之为“container runtime”，会做如下的事情:
